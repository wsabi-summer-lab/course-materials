...
base = evaluate_predictions("Training mean", "validation", y_valid, mean_valid)
test = evaluate_predictions("Training mean", "test", y_test, mean_test)
base_ll = base$log_loss
test_ll = test$log_loss
...
form_valid_eval <- evaluate_predictions(
  "Formation + alignment rate",
  "validation",
  y_valid,
  formation_valid
)

form_test_eval <- evaluate_predictions(
  "Formation + alignment rate",
  "test",
  y_test,
  formation_test
...
form_valid_ll <- form_valid_eval$log_loss
form_test_ll <- form_test_eval$log_loss
...
#The validation log loss isn't particularly strong at around .5
...
comparison = tibble(
    ...
    selected_tuning = c("none", "formation + alignment", "none", "train+validation", "train+validation"),
    validation_log_loss = c(base_ll, form_valid_ll, logistic_valid_log_loss, best_nnet_row$valid_log_loss , best_xgb_row$valid_log_loss ),
    test_log_loss = c(test_ll, form_test_ll, logistic_test_log_loss, nnet_test_log_loss, xgb_test_log_loss)
)
...
#XGBoost did the best here in terms of minimizing log loss. Trees seem to be useful for these, because there are certain factors like downs and formations that can easily be split on.  
...
#this does a better job of showing positioning on the field, but important factors like downs are less visible. A 3x3 filter would see the darkness, depending on where players are. Layers would have to be chosen. 
