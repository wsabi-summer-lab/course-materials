...
overall_rate = mean(train$playActionPass)
formation_order = train |>
  group_by(offenseFormation) |>
  summarise(rate = mean(playActionPass), .groups = "drop") |>
  arrange(rate) |>
  pull(offenseFormation)

formation_rates |>
  mutate(offenseFormation = factor(offenseFormation, levels = formation_order)) |>
  ggplot(aes(receiverAlignment, offenseFormation, fill = rate)) +
  geom_tile(color = "white") +
  geom_text(aes(label = paste0(sprintf("%.2f", rate), "\nn=", n)), size = 3) +
  scale_fill_gradientn(
    colours = c("#1a9850", "#ffffbf", "#d73027"),
    limits = c(0, max(0.6, max(formation_rates$rate))),
    name = "PA rate"
  ) +
  labs(
    x = "Receiver alignment",
    y = "Offense formation",
    title = "Training-data play-action rate by formation x alignment",
    subtitle = sprintf("Overall train PA rate = %.3f; cell = rate, n", overall_rate)
  ) +
  theme_minimal()
#I-form, wildcat have low play action rates, while shotgun, pistol are higher
###################################
...
comparison = tibble(
    ...
    selected_tuning = c(
        ...
        "formation + alignment",
        "none",
        sprintf("size = %g, decay = %g", best_nnet_row$size, best_nnet_row$decay),
        sprintf(
            "max_depth = %g, eta = %g, min_child_weight = %g",
            best_xgb_row$max_depth, best_xgb_row$eta, best_xgb_row$min_child_weight
        )
    ),
...
#XGBoost rates as the most accurate model. This could be due to the fact that categorical variables already give a lot of information on the play action probability, allowing XGBoost to split and theshold based on these
#Also, a small sample means that an NN might not capture the structure of the true data
####################################
