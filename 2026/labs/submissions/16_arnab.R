...
#The logistic validation log loss is .4298921, which is pretty high.
###########################
### SINGLE TREE TUNING ####
###########################
...
for (i in seq_len(nrow(xgb_validation))) {
  ...
  pred_valid <- predict(candidate, dvalid)
  
  xgb_validation$best_nrounds[i] <- 300
  xgb_validation$valid_log_loss[i] <- log_loss(valid$posteam_win, pred_valid)
}
...
final_xgb_params <- c(
  base_xgb_params,
  as.list(best_xgb[names(xgb_grid)])
)

xgb_fit <- xgb.train(
  params = final_xgb_params,
  data = dtrain_valid,
  nrounds = best_xgb$best_nrounds[[1]],
  verbose = 0
)
...
for (b in seq_len(B)) {
    ...
    boot_score_predictions[, b] <- predict(
      boot_fit,
      newdata = score_grid,
      type = "response"
    )
    
    boot_heatmap_predictions[, b] <- predict(
      boot_fit,
      newdata = heatmap_grid,
      type = "response"
    )
    
    boot_field_predictions[, b] <- predict(
      boot_fit,
      newdata = field_grid,
      type = "response"
    )
...
ggplot(field_intervals, aes(x = yardline_100, y = wp_hat)) +
  geom_ribbon(
    aes(ymin = wp_lo, ymax = wp_hi),
    alpha = 0.15
  ) +
  geom_line(linewidth = 1) +
  geom_hline(
