...
for (i in seq_len(nrow(xgb_validation))) {
  ...
  best_row = which.min(cv$evaluation_log$test_logloss_mean)
  xgb_validation$best_nrounds[i] = best_row
  xgb_validation$valid_log_loss[i] = cv$evaluation_log$test_logloss_mean[best_row]
}
...
comparison = tibble(
  ...
  validation_log_loss = c(logistic_valid_log_loss, best_tree$valid_log_loss, best_rf$valid_log_loss, best_xgb$valid_log_loss),
...
ggplot(heatmap_intervals, aes(game_seconds_remaining, score_differential)) +
  geom_tile(aes(fill = wp_hat)) +
  scale_fill_gradient2(low = "red", mid = "white", high = "blue", midpoint = 0.5) +
  scale_x_reverse() +
  labs(x = "Game seconds remaining", y = "Score differential", fill = "Win prob")

ggplot(heatmap_intervals, aes(game_seconds_remaining, score_differential)) +
  geom_tile(aes(fill = interval_width)) +
  scale_fill_viridis_c() +
  scale_x_reverse() +
  labs(x = "Game seconds remaining", y = "Score differential", fill = "95% CI width")
...
ggplot(field_predictions, aes(field_pos, wp_hat, color = factor(score_differential))) +
  geom_ribbon(aes(ymin = wp_lo, ymax = wp_hi, fill = factor(score_differential)), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1) +
  facet_grid(down ~ model) +
  labs(x = "Field position", y = "Win probability", color = "Score diff", fill = "Score diff")

#LR produces smooth 2 curve while the single tree is jarring steos, random forest, and XGboost have a smoother nonlinear hyet still rigid figure. 
#Tree based models automatically pick up on like game specific significances like 7 point lead inthe first quarter vs 4th quarter is a lot diffferent while that had to be encoded to the LR. Both hvae a lot of uncertainty early in the game beause blowouts early are rare so there is less signal here. 