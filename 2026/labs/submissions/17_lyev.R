...
mean_pred = mean(y_train) #0.291
mean_valid = rep(mean_pred, length(y_valid))
mean_test = rep(mean_pred, length(y_test))
evaluate_predictions("Training mean", "validation", y_valid, mean_valid) #logloss = 0.578
evaluate_predictions("Training mean", "test", y_test, mean_test) #logloss = 0.6
...
evaluate_predictions("Formation + alignment", "validation", y_valid, formation_valid)
#logloss = 0.585
evaluate_predictions("Formation + alignment", "test", y_test, formation_test)
#logloss = 0.59
...
ggplot(formation_rates,
       aes(x = receiverAlignment,
           y = offenseFormation,
           fill = rate)) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(name = "Play action rate", limits = c(0, 1)) +
  labs(
    title = "Play action rate by formation and receiver alignment (training)",
    x = "Receiver alignment",
    y = "Offense formation"
  ) +
  theme_minimal()

#I_form with 3x1 receiver alignment has by far the highest PA rate, alongside Jumbo 2x0
#Anything in wildcat, empty, or with 1x0 or 3x0 receiver alignment has a low PA rate
...
best_nnet_row
#Best size is 4 with logloss = 0.554
...
nnet_test_log_loss = log_loss(y_test, nnet_test) #logloss = 0.567
...
xgb_test_log_loss = log_loss(y_test, xgb_test) # 0.558
...
comparison = tibble(
  ...
  selected_tuning = c(
    ...
    "formation + alignment",
    "none",
    paste0("size=", best_nnet_row$size,
           ", decay=", best_nnet_row$decay),
    paste0("depth=", best_xgb_row$max_depth,
           ", eta=", best_xgb_row$eta,
           ", min_child_weight=", best_xgb_row$min_child_weight)
  ),
...
ggplot(comparison, aes(reorder(model, test_log_loss), test_log_loss)) +
    geom_col(width = 0.65, fill = "dodgerblue") +
...
example_grid = example_grid |>
  mutate(channel = fct_rev(channel))

ggplot(example_grid, aes(x_cell, y_cell, fill = value)) +
  geom_tile(width = 2, height = 2, color = "firebrick") +
...
#Information that isn't in this plot includes exact position of players and motion
#Information gained is a visual representation, which is hard to deduce from a tabular form only


#A 3x3 filter would see clumps of players in different zones, which could show how condensed formations are
#Choices before training would be the size of grid zones
