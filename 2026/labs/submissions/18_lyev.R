...
#PC1 seems to be more of a big role, posting up, rolling, and crashing
#PC2 seems to be more of a spot-up role
#What's interesting is that they are both significantly negative in P&R ball handling
...
ggplot(nba_clustered, aes(PC1, PC2, color = cluster)) +
  ...
  # Highlight the three target players
  geom_point(
    data = filter(nba_clustered, player_type == "Targets"),
    color = "#D95D39",
    size = 3
  ...
  # --- NEW: Add candidate players ---
  geom_point(
    data = canditates |> 
      left_join(nba_clustered |> select(player_szn, PC1, PC2), by = "player_szn"),
    color = "#2E86C1",
    size = 2.5
  ) +
  geom_text(
  ...
  labs(
    ...
    y = "PC2",
    color = "Cluster",
    title = "NBA K-means Clusters with Targets and Candidate Replacements"
  )


# First: attach cluster + PCA coordinates to candidates
candidates_aug <- canditates |>
  left_join(
    nba_clustered |> select(player_szn, cluster, PC1, PC2),
    by = "player_szn"
  ) |>
  left_join(
    nba_clustered |> select(player_szn, target_cluster = cluster),
    by = c("target_player_szn" = "player_szn")
  )

# Filter to same cluster as target + sort by distance
best_candidates <- candidates_aug |>
  group_by(team) |>
  arrange(distance) |>
  slice_head(n = 3) |>
  ungroup()
...
#For the spurs, I would recommend Kevin Porter Jr (2022)
#For the jazz, I would recommend Jalen Green (2022)
#For the pistons, I would recommend OG Anunoby (2021)
#These are all based on euclidean distance
