...
pa = read_csv("C:/Users/sundw/Downloads/17_play-action-vs-run.csv", show_col_types = FALSE) |>
    mutate(
...
snap_examples = read_csv("C:/Users/sundw/Downloads/17_snap-examples.csv", show_col_types = FALSE) |>
    mutate(
        playActionPass = as.integer(playActionPass),
...
pa |> count(split, playActionPass) |> group_by(split) |> mutate(pct = n / sum(n))

# split      playActionPass     n   pct
# <fct>               <int> <int> <dbl>
# 1 train                   0  4189 0.709
# 2 train                   1  1723 0.291
# 3 validation              0   644 0.737
# 4 validation              1   230 0.263
# 5 test                    0  1279 0.713
# 6 test                    1   516 0.287
...
ggplot(formation_rates, aes(x = receiverAlignment, y = offenseFormation, fill = rate)) +
  ...
  labs(
    ...
    title = "Play-action rate by formation and receiver alignment (training data)"
  ) +
  theme_minimal() +
...
comparison = tibble(
  ...
  selected_tuning = c(
    ...
    "formation + alignment",
    "none",
    sprintf("size = %d, decay = %s", best_nnet_row$size, best_nnet_row$decay),
    sprintf("max_depth = %d, eta = %s, min_child_weight = %d, nrounds = %d",
            best_xgb_row$max_depth, best_xgb_row$eta,
            best_xgb_row$min_child_weight, best_xgb_row$best_iteration)
  ),
  validation_log_loss = c(
    log_loss(y_valid, mean_valid),
    log_loss(y_valid, formation_valid),
    logistic_valid_log_loss,
    best_nnet_row$valid_log_loss,
    best_xgb_row$valid_log_loss
  ),
  test_log_loss = c(
    log_loss(y_test, mean_test),
    log_loss(y_test, formation_test),
...
ggplot(example_grid, aes(x_cell, y_cell, fill = value)) +
   geom_tile(width = 2, height = 2, color = "red") +
...
example_keys = snap_examples |>
  distinct(gameId, playId, playActionPass) |>
  group_by(playActionPass) |>
  slice(1) |>
  ungroup()

example_plays = snap_examples |>
  semi_join(example_keys, by = c("gameId", "playId")) |>
  mutate(x_rel = xStd - lineOfScrimmage)

ggplot(example_plays, aes(x = x_rel, y = yStd)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "#f4d35e", linewidth = 1) +
  geom_point(
    data = filter(example_plays, side != "football"),
    aes(color = side), size = 4
  ) +
  geom_point(
    data = filter(example_plays, side == "football"),
    shape = 21, fill = "#6b4423", color = "#3e2712", size = 3
  ) +
  facet_wrap(
    ~ playActionPass, nrow = 1,
    labeller = labeller(playActionPass = c(`0` = "No play-action", `1` = "Play-action"))
  ) +
  scale_color_manual(values = c(offense = "#2c6fbb", defense = "#d65a31")) +
  coord_fixed(xlim = c(-10, 18), ylim = c(0, 160 / 3), expand = FALSE) +
  labs(
    x = "Yards from line of scrimmage",
    y = "Field width (yd)",
    color = NULL,
    title = "Pre-snap player positions: play-action vs. non-play-action"
  ) +
  theme_minimal() +
  theme(
    panel.background = element_rect(fill = "#3f7d3f", color = "white"),
