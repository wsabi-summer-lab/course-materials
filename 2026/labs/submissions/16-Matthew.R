...
fit_best_model = function(data, model_name) {
  switch(
    model_name,
    "Logistic model" = glm(logistic_formula, data = data, family = binomial()),
    "Classification tree" = fit_tree(data, best_tree$maxdepth, best_tree$minbucket),
    "Random forest" = fit_rf(data, mtry = best_rf$mtry, min_node_size = best_rf$min_node_size, num_trees = 600),
    "XGBoost" = xgb.train(
      params = final_xgb_params,
      data = xgb.DMatrix(make_xgb_matrix(data), label = data$posteam_win),
      nrounds = best_xgb$best_nrounds,
      verbose = 0
    )
  )
...
predict_best_model = function(fit, grid, model_name) {
  switch(
    model_name,
    "Logistic model" = predict(fit, grid, type = "response"),
    "Classification tree" = predict(fit, grid, type = "prob")[, "1"],
    "Random forest" = predict(fit, grid)$predictions[, "1"],
    "XGBoost" = predict(fit, xgb.DMatrix(make_xgb_matrix(grid)))
  )
}
...
for (b in seq_len(B)) {
  ...
  boot_fit = fit_best_model(boot_data, best_model_name)
  
  boot_score_predictions[, b] = predict_best_model(boot_fit, score_grid, best_model_name)
  boot_heatmap_predictions[, b] = predict_best_model(boot_fit, heatmap_grid, best_model_name)
  boot_field_predictions[, b] = predict_best_model(boot_fit, field_grid, best_model_name)
}
...
ggplot(heatmap_intervals, aes(game_seconds_remaining, score_differential, fill = wp_hat)) +
  geom_tile() +
  scale_fill_gradient2(midpoint = 0.5, limits = c(0, 1), low = "firebrick", high = "steelblue", mid = "white") +
  labs(
...
ggplot(heatmap_intervals, aes(game_seconds_remaining, score_differential, fill = interval_width)) +
  geom_tile() +
  scale_fill_viridis_c() +
  labs(
    title = "95% bootstrap interval width",
    x = "Game seconds remaining",
    y = "Possession-team score differential",
    fill = "Interval width"
  )

field_predictions = field_intervals |>
  mutate(score_differential = factor(score_differential))

ggplot(field_predictions, aes(field_pos, wp_hat, color = score_differential, fill = score_differential)) +
  geom_ribbon(aes(ymin = wp_lo, ymax = wp_hi), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1) +
  facet_wrap(~ down) +
  geom_hline(yintercept = 0.5, color = "gray60", linetype = "dashed") +
  labs(
    title = paste("Predicted win probability —", best_model_name),
    x = "Field position (yards from opponent end zone)",
    y = "Predicted win probability",
