...
feature_summary = shots |>
  group_by(goal) |>
  summarize(
    mean_distance = mean(distance_to_goal, na.rm = TRUE),
    median_distance = median(distance_to_goal, na.rm = TRUE),
    
    mean_angle = mean(abs_angle_to_goal, na.rm = TRUE),
    median_angle = median(abs_angle_to_goal, na.rm = TRUE),
    
    mean_open_goal = mean(open_goal_share, na.rm = TRUE),
    
    mean_defender = mean(nearest_defender_distance, na.rm = TRUE),
...
#Based on this, it seems like nearest defender distance and absolute 
#distance matters the most. 
...
kde_small  = make_kde(shots, h = 1.5, label = "h = 1.5")
kde_medium = make_kde(shots, h = 3.5, label = "h = 3.5")
kde_large  = make_kde(shots, h = 7.0, label = "h = 7.0")
...
ggplot(kde_all,
    ...
    labs(
        title = "Kernel Density Estimates of Shot Locations",
        fill = "Density"
    ) +
    theme_pitch
...
ggplot() +
  ...
  labs(
    title = "Location-only Expected Goals"
  ) +
