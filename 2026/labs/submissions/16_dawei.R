...
wp = read_csv("C:/Users/sundw/Downloads/16_win-probability.csv", show_col_types = FALSE) |>
    mutate(
        down = factor(down),
        qtr = factor(qtr),
        half = factor(half),
...
for (i in seq_len(nrow(xgb_validation))) {
  ...
  eval_log = attributes(candidate)$evaluation_log
  metric_col = setdiff(names(eval_log), "iter")[1]
  best_idx = which.min(eval_log[[metric_col]])
  
  xgb_validation$best_nrounds[i] = eval_log$iter[best_idx]
  xgb_validation$valid_log_loss[i] = eval_log[[metric_col]][best_idx]
}
...
comparison = tibble(
  ...
  selected_params = c(
    "—",
    paste0("maxdepth=", best_tree$maxdepth, ", minbucket=", best_tree$minbucket),
    paste0("mtry=", best_rf$mtry, ", min_node_size=", best_rf$min_node_size),
    paste0(
      "eta=", best_xgb$eta,
...
best_model_name = "Logistic Model"
...
for (b in seq_len(B)) {
  ...
  if (best_model_name == "Logistic model") {
    boot_fit = glm(logistic_formula, data = boot_data, family = binomial())
    boot_score_predictions[, b]   = predict(boot_fit, score_grid, type = "response")
    boot_heatmap_predictions[, b] = predict(boot_fit, heatmap_grid, type = "response")
    boot_field_predictions[, b]   = predict(boot_fit, field_grid, type = "response")
    
  } else if (best_model_name == "Classification tree") {
    boot_fit = fit_tree(boot_data, maxdepth = best_tree$maxdepth, minbucket = best_tree$minbucket)
    boot_score_predictions[, b]   = predict(boot_fit, score_grid, type = "prob")[, "1"]
    boot_heatmap_predictions[, b] = predict(boot_fit, heatmap_grid, type = "prob")[, "1"]
    boot_field_predictions[, b]   = predict(boot_fit, field_grid, type = "prob")[, "1"]
    
  } else if (best_model_name == "Random forest") {
    boot_fit = fit_rf(boot_data, mtry = best_rf$mtry, min_node_size = best_rf$min_node_size, num_trees = 600)
    boot_score_predictions[, b]   = predict(boot_fit, score_grid)$predictions[, "1"]
    boot_heatmap_predictions[, b] = predict(boot_fit, heatmap_grid)$predictions[, "1"]
    boot_field_predictions[, b]   = predict(boot_fit, field_grid)$predictions[, "1"]
    
  } else if (best_model_name == "XGBoost") {
    d_boot = xgb.DMatrix(make_xgb_matrix(boot_data), label = boot_data$posteam_win)
    boot_fit = xgb.train(params = final_xgb_params, data = d_boot, nrounds = best_xgb$best_nrounds, verbose = 0)
    boot_score_predictions[, b]   = predict(boot_fit, xgb.DMatrix(make_xgb_matrix(score_grid)))
    boot_heatmap_predictions[, b] = predict(boot_fit, xgb.DMatrix(make_xgb_matrix(heatmap_grid)))
    boot_field_predictions[, b]   = predict(boot_fit, xgb.DMatrix(make_xgb_matrix(field_grid)))
  }
}
...
predict_best_model = function(grid) {
  model_key = tolower(best_model_name)
  
  if (model_key == "logistic model") {
    predict(final_logistic_fit, grid, type = "response")
  } else if (model_key == "classification tree") {
    predict(tree_fit, grid, type = "prob")[, "1"]
  } else if (model_key == "random forest") {
    predict(rf_fit, grid)$predictions[, "1"]
  } else if (model_key == "xgboost") {
    predict(xgb_fit, xgb.DMatrix(make_xgb_matrix(grid)))
  } else {
    stop("best_model_name ('", best_model_name, "') didn't match any known model class.")
  }
}

### Heatmap for the best-performing model ###

heatmap_predictions = heatmap_grid |>
  mutate(wp_hat = predict_best_model(heatmap_grid)) |>
  left_join(
    heatmap_intervals |> select(score_differential, game_seconds_remaining, interval_width),
    by = c("score_differential", "game_seconds_remaining")
  )

print(
  ggplot(heatmap_predictions, aes(score_differential, game_seconds_remaining, fill = wp_hat)) +
    geom_tile() +
    scale_fill_viridis_c(name = "Win probability", limits = c(0, 1)) +
    labs(
      title = paste("Predicted win probability —", best_model_name),
      x = "Possession-team score differential",
...
print(
  ggplot(heatmap_predictions, aes(score_differential, game_seconds_remaining, fill = interval_width)) +
    geom_tile() +
    scale_fill_viridis_c(name = "95% interval width", option = "magma") +
    labs(
      title = paste("Bootstrap interval width —", best_model_name),
      x = "Possession-team score differential",
      y = "Game seconds remaining"
    )
)

### Field-position plot, faceted by model and down ###

field_predictions = bind_rows(
  field_grid |> mutate(model = "Logistic model", wp_hat = predict(final_logistic_fit, field_grid, type = "response")),
  field_grid |> mutate(model = "Classification tree", wp_hat = predict(tree_fit, field_grid, type = "prob")[, "1"]),
  field_grid |> mutate(model = "Random forest", wp_hat = predict(rf_fit, field_grid)$predictions[, "1"]),
  field_grid |> mutate(model = "XGBoost", wp_hat = predict(xgb_fit, xgb.DMatrix(make_xgb_matrix(field_grid))))
) |>
  left_join(
  ...
  mutate(
    wp_lo = if_else(tolower(model) == tolower(best_model_name), wp_lo, NA_real_),
    wp_hi = if_else(tolower(model) == tolower(best_model_name), wp_hi, NA_real_)
  )

ggplot(field_predictions, aes(field_pos, wp_hat, color = factor(score_differential))) +
  geom_ribbon(
    aes(ymin = wp_lo, ymax = wp_hi, fill = factor(score_differential)),
    alpha = 0.15,
    color = NA
  ) +
  geom_line(linewidth = 1) +
  facet_grid(down ~ model) +
  geom_hline(yintercept = 0.5, color = "gray60", linetype = "dashed") +
  labs(
