...
ggplot(shots, aes(distance_to_goal, fill = factor(goal))) +
  geom_density(alpha = 0.45) +
  scale_fill_manual(values = c("0" = "gray80", "1" = "#D95D39"),
                    name = "Goal") +
  labs(
    title = "Distribution of Shot Distance by Outcome",
    x = "Distance to Goal (meters)"
  ) +
  theme_minimal()


ggplot(shots, aes(open_goal_share, fill = factor(goal))) +
  geom_density(alpha = 0.45) +
  scale_fill_manual(values = c("0" = "gray80", "1" = "#D95D39"),
                    name = "Goal") +
  labs(
    title = "Open Goal Share by Outcome",
    x = "Open Goal Share"
  ) +
  theme_minimal()


ggplot(shots, aes(nearest_defender_distance, fill = factor(goal))) +
  geom_density(alpha = 0.45) +
  scale_fill_manual(values = c("0" = "gray80", "1" = "#D95D39"),
                    name = "Goal") +
  labs(
    title = "Nearest Defender Distance by Outcome",
    x = "Distance to Nearest Defender (m)"
  ) +
  theme_minimal()
...
kde_data = map2_dfr(
  bandwidths,
  bandwidths,
  \(h, label) make_kde(shots, h, paste0("h = ", h))
)

kde_data = map_dfr(
  bandwidths,
  \(h) make_kde(shots, h, paste0("h = ", h))
)

ggplot(kde_data, aes(shot_x, shot_y, z = density)) +
  pitch_background() +
  ...
  labs(
    title = "Shot Location KDEs for Multiple Bandwidths",
    fill = "Density"
  ) +
  theme_pitch
...
location_test_log_loss = log_loss(test$goal, location_test_pred) #0.345
...
xg_grid$pred = nw_predict_scaled(
  train_validation |> select(shot_x, shot_y),
  train_validation$goal,
...
ggplot() +
  ...
  geom_raster(
    data = xg_grid,
    aes(shot_x, shot_y, fill = pred),
    alpha = 0.9
  ) +
  pitch_lines() +
  coord_fixed(xlim = c(0, 68), ylim = c(70, 105.8), expand = FALSE) +
  scale_fill_viridis_c(option = "C", name = "xG") +
  labs(
    title = "Location-Only Expected Goals (xG)",
    subtitle = paste0("Kernel regression with bandwidth h = ", best_location_h)
  ) +
  theme_pitch
...
rich_test_log_loss = log_loss(test$goal, rich_test_pred) # 0.337
