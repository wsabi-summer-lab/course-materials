...
ggplot(shots, aes(x = factor(goal), y = distance_to_goal, fill = factor(goal))) +
  ...
  scale_fill_manual(values = c("0" = "steelblue", "1" = "#D95D39"), 
                    labels = c("No Goal", "Goal"), name = "") +
  scale_x_discrete(labels = c("No Goal", "Goal")) +
  labs(
    title = "Distance to Goal by Outcome",
    x = NULL,
...
#Distance to Goal definitely makes a difference towards Goals and not Goals.
...
kde_data = bind_rows(
  make_kde(shots, h = 1.5, label = "Small (h=1.5)"),
  make_kde(shots, h = 4,   label = "Medium (h=4)"),
  make_kde(shots, h = 10,  label = "Large (h=10)")
)

ggplot(kde_data, aes(shot_x, shot_y, fill = density)) +
  geom_tile() +
  facet_wrap(~bandwidth) +
  scale_fill_viridis_c() +
  coord_fixed() +
  labs(title = "Shot Density by Bandwidth") +
  theme_void()

#A small bandwidth or h causes the surface to be more grainy and noisy and the higher the bandwidth the smoother the clustering is.
#the small bandwidth is too grainy on the outside and the parameter of the clustering. The high bandwidth function is too smooth near the peaks.
...
# TODO: Compute validation log loss for each location bandwidth.
...
# TODO: Fit on train + validation, then evaluate test log loss.
...
# TODO: Make a pitch heatmap of location only xG.
...
ggplot() +
  pitch_background() +
  geom_tile(data = xg_grid, aes(shot_x, shot_y, fill = xg), alpha = 0.85) +
  pitch_lines() +
  scale_fill_gradientn(
    colors = c("#1a1a2e", "#16213e", "#f5a623", "#e74c3c"),
    name = "xG"
  ) +
  coord_fixed(xlim = c(0, 68), ylim = c(70, 105.8), expand = FALSE) +
  labs(title = "Location-Only xG Surface") +
  theme_pitch

#the heatmap shows a significant increase in xG when closer to the goal which makes sense.
...
ggplot() +
  geom_point(
    aes(x = location_test_pred, y = rich_test_pred, color = factor(test$goal)),
    alpha = 0.4, size = 0.8
  ) +
  geom_abline(slope = 1, intercept = 0, color = "black", linetype = "dashed") +
  scale_color_manual(values = c("0" = "steelblue", "1" = "#D95D39"),
                     labels = c("No Goal", "Goal"), name = "") +
  labs(
    title = "Location-Only vs Tracking Feature xG",
    x = "Location-Only xG",
    y = "Rich Feature xG"
  ) +
  theme_minimal(base_size = 12)

#Most of the shots go up are goals since some factors like no defender in the way can increase the xG significantly. Mostly the shots that go down are misses and these could be going down because of well defended players. 
